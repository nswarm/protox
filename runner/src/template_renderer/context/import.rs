...
impl ImportContext {
    pub fn new(relative_path: &Path, config: &RendererConfig) -> Result<Self> {
        let context = Self {
...
mod tests {
    ...
    fn file_path() -> Result<()> {
        ...
        let context = ImportContext::new(&PathBuf::from(path), &RendererConfig::default())?;
    ...
    fn package_with_configured_separator() -> Result<()> {
        ...
        let context = ImportContext::new(&PathBuf::from(path), &config)?;
        assert_eq!(context.package, "root::sub");
        assert_eq!(context.package_with_file, "root::sub::file_name");
        Ok(())
    }
