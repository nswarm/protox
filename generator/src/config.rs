...
pub const SCRIPT_OVERLAY: &str = "script-overlay";
    ...
    App::new(APP_NAME)
        ...
        .args([
            ...
            Arg::new(SCRIPT)
                .display_order(display_order())
                .long_help(join_help(&[
                    &format!("Code will be generated for the scripts and configuration found inside a folder called name inside --{}, and written to a folder called NAME inside --{}.", SCRIPT_ROOT, OUTPUT_ROOT),
                    &format!("Must also use either --{}, or --{} with an absolute path.", SCRIPT_ROOT, SCRIPT_IN),
                    &format!("Must also use either --{}, or --{} with an absolute path.", OUTPUT_ROOT, SCRIPT_OUT),
                    "Scripts use the language rhai (https://rhai.rs/).",
                    &format!("See also: --{}.", SCRIPT_OVERLAY),
                ]).as_str())
                .default_short()
                .long(SCRIPT)
                .value_names(&["NAME"])
            ...
            Arg::new(SCRIPT_ROOT)
                .display_order(display_order())
                .help(format!("All non-absolute --{} paths will be prefixed with this path. Required if any --{} INPUT paths are relative.", SCRIPT_IN, SCRIPT_IN).as_str())
                .long(SCRIPT_ROOT)
                .takes_value(true),
            ...
            Arg::new(SCRIPT_IN)
                .display_order(display_order())
                .long_help(join_help(&[
                    &format!("Overrides the input dir of generated code defined by --{} NAME to be INPUT.", SCRIPT),
                    &format!("e.g. --{} my_name --{} my_name path/to/input/folder", SCRIPT, SCRIPT_IN),
                    &format!("If INPUT is a relative path, it is evaluated relative to --{}.", SCRIPT_ROOT),
                ]).as_str())
                .long(SCRIPT_IN)
                .value_names(&["NAME", "INPUT"]),

            Arg::new(SCRIPT_OUT)
                .display_order(display_order())
                .long_help(join_help(&[
                    &format!("Overrides the output dir of generated code defined by --{} NAME to be OUTPUT.", SCRIPT),
                    &format!("e.g. --{} my_name --{} my_name path/to/output/folder", SCRIPT, SCRIPT_OUT),
                    &format!("If OUTPUT is a relative path, it is evaluated relative to --{}.", OUTPUT_ROOT),
                ]).as_str())
                .long(SCRIPT_OUT)
                .value_names(&["NAME", "OUTPUT"]),

            Arg::new(SCRIPT_OVERLAY)
                .display_order(display_order())
                .long_help(join_help(&[
                    &format!("Specify the path to one or more Overlay configuration files for the code generated to be generated by --{} NAME.", SCRIPT),
                    "These files must be in yaml or json, and follow the same rules as the main configuration \"overlays\" field. The overlays will be applied in the order specified, duplicate entries will be overridden.",
                    &format!("If PATH is a relative path, it is evaluated relative to --{}.", SCRIPT_ROOT),
                ]).as_str())
                .long(SCRIPT_OVERLAY)
...
fn parse_script_configs(
    ...
    output_root: Option<&PathBuf>,
) -> Result<Vec<ScriptConfig>> {
    let mut configs = Vec::new();
    let scripts = match args.grouped_values_of(SCRIPT) {
        None => return Ok(configs),
        Some(values) => values,
    };
...
fn get_grouped_values_by_name<'a>(
    args: &'a ArgMatches,
    arg_name: &str,
    name: &str,
) -> Vec<&'a str> {
    let grouped = args.grouped_values_of(arg_name);
    if grouped.is_none() {
        return Vec::new();
    }
    ...
    for value in grouped {
        ...
        if value.get(0).unwrap() == &name {
            values.push(*value.get(1).unwrap());
        }
    }
    values
}
