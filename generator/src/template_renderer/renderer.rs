...
mod tests {
    ...
    mod generated_header {
        ...
        fn default_in_file() -> Result<()> {
            let test_dir = tempdir()?;
            render_with_config(test_dir.path(), RendererConfig::default())?;
        ...
        fn default_in_single_file_package() -> Result<()> {
            ...
            let mut config = RendererConfig::default();
            config.one_file_per_package = true;
            render_with_config(test_dir.path(), config)?;
            assert_file_has_header(&test_dir.path().join("root"), DEFAULT_GENERATED_HEADER)?;
        ...
        fn render_with_config(path: &Path, config: RendererConfig) -> Result<(), Error> {
            let descriptor_set = FileDescriptorSet {
                file: vec![
            ...
            let mut renderer = Renderer::with_config(config);
            renderer.load_template_string("file", "")?;
            renderer.load_template_string("metadata", "")?;
